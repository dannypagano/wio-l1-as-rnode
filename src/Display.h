...
#define REFRESH_PERIOD 300000 // 5 minutes in ms
  ...
  #elif BOARD_MODEL == BOARD_TBEAM_S_V1 || BOARD_MODEL == BOARD_WIO_TRACKER_L1
...
#if BOARD_MODEL == BOARD_TBEAM_S_V1 || BOARD_MODEL == BOARD_WIO_TRACKER_L1
...
bool display_init() {
      ...
      Wire.begin(SDA_OLED, SCL_OLED);
    #elif BOARD_MODEL == BOARD_WIO_TRACKER_L1
      // nRF52's TwoWire does not support begin(sda, scl) the way ESP32's does
      // (see the T-Beam Supreme arm above, which IS an ESP32 board) -- pins
      // must be set separately before a plain begin() call.
